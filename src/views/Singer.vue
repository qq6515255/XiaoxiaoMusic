<template>
  ...
  >
    ...
    <div v-if="playlist" class="img-box" :style="'background-image:url(' + bgimg + ')'">
      ...
      <div class="listInfo">
        <h2 class="title">{{ singerInfo.name }} - {{ singerInfo.alias[0]}}</h2>
        <span class="fa fa-headphones"></span>
        <span class="playCount">歌曲:{{ singerInfo.musicSize }}首</span><br>
        <span class="fa fa-caret-square-o-right"></span>
        <span class="playCount">专辑:{{ singerInfo.albumSize }}部</span>
        <p class="description">{{ singerInfo.briefDesc }}</p>
      </div>
    </div>
    <!-- 歌曲列表部分 -->
    <ul class="songlist">
      <template v-if="playlist">
        <SongListItem
          v-for="(item, index) in playlist.slice(0, listLength)"
          :key="index"
          :song-item="item"
...
export default {
  ...
  computed: {
    bgimg() {
      if (this.singerInfo.picUrl === null) {
  ...
  methods: {
    ...
    getPlayListDetail(id) {
      this.axios
        .get("http://junnaix.cn:7878/artists", {
          params: {
            id
          }
        })
        .then(reponse => {
          ...
          ({ artist: this.singerInfo,hotSongs:this.playlist } = reponse.data);
        });
    },
    // 播放数转化
